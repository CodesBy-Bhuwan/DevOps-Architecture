...
##### 
#output "docker_target_public_ip" {
#  value = aws_instance.docker_target.public_ip
#}

#output "docker_target_private_ip" {
#  value = aws_instance.docker_target.private_ip
#}

output "k8s_master_public_ip" {
  value = join("", aws_instance.k8s_master[*].public_ip)
}
output "k8s_master_private_ip" {
  value = join("", aws_instance.k8s_master[*].private_ip)
}
output "k8s_workers_public_ips" {
  value = aws_instance.k8s_workers[*].public_ip
}
...
output "docker_target_public_ip" {
  value = join("", aws_instance.docker_target[*].public_ip)
}
output "docker_target_private_ip" {
